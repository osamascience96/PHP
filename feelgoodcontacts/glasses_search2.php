...
$attt = 1;
	...
	$sql="SELECT * FROM product  ";//Query stub
		...
		else if(isset($_POST['lensecolor'])) {
        	foreach($_POST['lensecolor'] as $c){
				if(!empty($c)){
					$sql .= $clause."Lens_Colour=".$c."";
					$clause = " OR ";//Change  to OR after 1st WHERE
					$fetch_query = mysqli_query($conn,$sql);
				}   
		...
		else if(isset($_POST['lensesize'])) {
        	foreach($_POST['lensesize'] as $c){
				if(!empty($c)){
					$sql .= $clause."Lens_Size=".$c."";
					$clause = " OR ";//Change  to OR after 1st WHERE
					$fetch_query = mysqli_query($conn,$sql);
				}   
			...
			if(!empty($_SESSION['favourite_cart']))
                    ...
                    <?php $attt++; ?>
						...
						<div class="product_colors_div col-md-12 col-sm-12 col-lg-12">
							...
							//Count Table Row
							$results3 = $conn->query("SELECT COUNT(*) FROM product_merge where Product_id=$temp_id AND active=1");
							$rows3 = $results3->fetch_row();
							if($rows3[0] != 0) {
